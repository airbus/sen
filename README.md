...
You need [Conan](https://conan.io/), a C++17 compiler (GCC, Clang, Visual Studio), CMake, Ninja and
pkg-config; the last three are used by the third-party recipes when they build from source. Ninja
...
folder, because several of the profiles include a shared base:

```shell
conan config install -tf profiles .conan/profiles/
```

Then build with `sen_gcc`, which follows the machine you run it on and so suits Intel, AMD
and 64-bit arm alike. It targets Linux and pins `gcc-12`, which has to exist on your machine.
...
- Windows is supported and in use, and its tests run on every pull request, though not yet at parity
  with Linux: the kernel integration tests and the suites that need a Linux container, signals or
  `/proc` are excluded there.
