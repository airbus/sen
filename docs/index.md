...
- [Getting started](getting_started/index.md) takes you from nothing installed to your own package
  running.
- [Tutorial 1: Hello Sen](tutorials/hello_sen.md) is the shortest path to something running, and
...
- The [how-to guides](howto_guides/index.md) each answer one question, and the
  [components](components/index.md) pages cover what ships with Sen.
...
- [*Ethernet transport*](components/ether.md) that supports asynchronous I/O over TCP, UDP unicast
  and multicast.
- [*Recorder*](components/recording.md), highly customizable, with LZ4 compression, indexes,
  snapshots, annotations, etc.
- [*Replayer*](components/replaying.md) with support for real-time, stepped execution and random
  access.
- [*Python interpreter*](components/py.md) that can be instantiated. You can script your components
  and tests.
- [*Shell*](components/shell.md) for CLI interaction, with auto-completion, introspection, and
  remote connectivity.
- [*Grafana visualization*](components/influx.md) via the InfluxDB component.
- [*Explorer GUI*](components/explorer.md) to inspect and interact with your system (objects,
  events, sessions, plots).
- [*Web explorer*](components/webexplorer.md) serving the same thing from Sen itself, to a browser.
- [*JSON-RPC*](components/jsonrpc.md) 2.0 over WebSocket, with a
  [TypeScript client](components/jsonrpc_ts_client.md) for browser and Node applications.
- [*MCP gateway*](components/mcp_gateway.md) so a large language model can observe and drive a
  running system, or read a recording.
- [*Log master*](components/logmaster.md) to have full control over your logs.
- [*Tracer*](components/tracy.md) to use the frame-based tracer
  [Tracy](https://github.com/wolfpld/tracy) to inspect the behavior of your processes.
- [*REST*](components/rest.md) API to interface with Sen through HTTP endpoints.
