...
A configuration file has up to four top-level keys. All of them are optional, and a file with none
of them starts a kernel that does nothing.
...
the part of configuring Sen where the tooling already does the work, so it is worth the five minutes
to wire up.
