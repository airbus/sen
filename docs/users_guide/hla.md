...
Or a browsable reference over the whole FOM, which is usually the quickest way to find a name in
one:
...
[Code generation](code_generation.md) covers the rest of what Sen writes from a FOM.
...
[generated FOM reference](../examples/generated_reference.md) lists every name in this form, which
is the quickest way to find the one you want.

So the same class is written differently depending on which file you are in:
...
**The mappings file decides the interface, the settings file decides the implementation.** Whether
an interaction becomes a method or an event, what its arguments are, and
...
Within one process stepping is deterministic, and across processes it is not yet. And a component
that cannot be stepped keeps following the real clock, `ether` among them, because the network does
not step.
