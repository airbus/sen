...
adapts rather than refuses, and some of those adaptations lose data: numbers clamp, sequences
truncate. Under the default `relaxed` compatibility each of those is warned about, and `strict`
refuses the conversion instead. [Run-time compatibility](compatibility_conversions.md) has the
settings, and is worth reading if your FOM is shared across organizations.
...
[the execution model](execution_model.md#the-time-a-component-sees). If your model needs a time of
...
- HLA Records are mapped to Sen structs. A record that draws its fields from another with
  `<include>` becomes a struct extending that one, which is what Sen's single inheritance is for.
