...
how far time moves on each step is the distance to the next component due to run.
...
The step size is not a setting. Each kernel clock reports how long until the next component it hosts
is due, and the master takes the smallest, so time stops at the next thing that has to run. Steps
therefore vary in length, and a component's `freqHz` still decides how often it updates.
