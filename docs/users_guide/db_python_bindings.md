...
- **properties[i]**: `name`, `description`, `category` (`staticRO` / `staticRW` / `dynamicRO` / `dynamicRW`), `type`, `transportMode`, `tags`, `checkedSet`.
- **methods[i]**: `name`, `description`, `args` (list of `{name, description, type}`), `transportMode`, `constness`, `deferred`, `returnType`, `propertyRelation`, `localOnly`.
