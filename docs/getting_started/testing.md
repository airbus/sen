...
```shell
conan install . --profile=sen_gcc -o "sen/*:with_tests=True" --build=missing
conan build   . --profile=sen_gcc
...
```shell
conan install . --profile=sen_gcc -o "sen/*:with_tests=True" -s build_type=Debug --build=missing
