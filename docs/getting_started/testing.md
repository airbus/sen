...
| kernel `transport` / `runtime_compatibility` / `crash_report` / `type_clash` | Enabled everywhere. They drive several `sen run` processes through `runner.py`, so they need no container, but they do need the `ether` and `py` components in the build. |
