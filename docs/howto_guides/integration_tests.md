...
### The mode changes what the tool is

`setRunMode` chooses the mode. Set it before adding any kernel, because it applies to kernels built
after it, and one already added keeps what it was built with.
...
for the kernel by polling state instead of by counting steps.

In **real time**, the kernels free-run on their own threads. Runs differ from each other, which is
what makes this the mode for interleavings and races. Stepping cannot produce those, because it does
not let two components run at once.
...
The comment matters. Components must outlive the cluster because the kernels call into them while
shutting down, and subscription and source handles must be released before the cluster goes.
Declaration order is how you get both, so it is the first thing to write and the easiest thing to
get wrong.
...
The budget is either a step count or elapsed wall-clock time, and picking the wrong one costs you. A
...
transport does not drop messages just because the far side is busy. The distinction matters, because
a peer that is truly gone is a disconnection and takes an entirely different path through the
kernel. There is no other way to stage silence, since a genuinely hung process would hang your test
with it.

**Hiding from discovery.** `hideFromDiscovery` keeps one process out of what another can find while
leaving the traffic between them untouched. That combination is the real start-up window, because a
session's transport delivers from the moment it exists and the kernel server is told about the
remote kernel separately and later. `showInDiscovery` closes the window again.

**Forging a membership event.** The network can deliver a participant-joined announcement carrying
field values of your choosing, as though a kernel had announced itself. That gives you a way to test
against adversarial input, and not only against its absence. This is the least settled part of the
surface, so expect the call itself to be reshaped before it ships.
...
finish on a count instead of a timeout. The last one separates a cancelled timer from an abandoned
...
One thing the network will not do is lose a message on the reliable channel. That is a decision and
not a gap. A reliable transport does not lose messages, so a dropped one would model nothing that
can happen. Loss that can happen belongs to the best-effort plane.
