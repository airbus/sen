...
its threads, the work it does reclaiming memory. The vocabulary is the kernel's, and none of it
describes your model. That makes this an advanced surface. It is also where the answer is when a
system behaves strangely and the cause is not in your own code, and there is a lot of it.
...
kernel thread then gets its own tracer, so a tracer needs no locking of its own. The factory is
called from each thread as that thread starts, so an implementation that sets up a backend lazily
has to make that setup thread-safe.

An event is a name and a list of typed key-value fields. **The keys and any string values are
borrowed views, valid only for the duration of the call**, which is easy to miss. A tracer that
stores an event must copy first.
...
mark, so a system sitting just over the line reports once per episode instead of continuously. The
subject is the thing being measured, so you get one report per lane, per remote kernel, per
...
to the other five. For those five, a tracer is the only way to see them at all.

## What the numbers mean

Some of these carry a signal on their own.
...
batch, which is a memory problem and not a rate one. Bytes are the honest signal for an unbounded
queue, because counting entries misses the case of a few enormous ones.

**The server thread's busy fraction** is the capacity number. Sustained above one half means half
the single thread's budget is gone and the ceiling is close enough to plan for. At one it is
saturated, and the queue behind it can only grow.
...
Others matter once something is already wrong. **Latency by kind of work** is the follow-up to a
queue warning, because it names which work is slow, and that is what turns an alert into a
