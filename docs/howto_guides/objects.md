...
A method can be marked deferred directly in the STL, with the `[deferred]` attribute:

```rust
fn computeFibonacci(n: u32) -> u64 [deferred];
```

It can also be marked in the code generation settings, which keeps the STL free of implementation
choices, so the same interface can be implemented with or without deferral. That is what the `fibonacci` example
