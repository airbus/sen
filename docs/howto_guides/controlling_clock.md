...
The step size is not yours to set. Each kernel clock reports how long until the next component it
hosts is due to run, and the master takes the smallest of those replies. So time stops at the next
thing that has to run rather than advancing by a fixed amount, and steps are not all the same
length. Components keep their own `freqHz`: with one at 100 Hz and one at 10 Hz, time stops every
10 ms, and the slower one runs on every tenth of those stops.

## Stepping the time

1. Start your kernel in virtualized time mode.
2. Tell it to instantiate a clock by setting `clockMaster` to `true`.
3. Find the "master" object and call `step()`, `steps(n)` or `advanceTime(duration)` on it.
