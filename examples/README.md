...
```shell
conan install . --profile:all=sen_gcc --build=missing -o "sen/*:with_examples=True"
conan build . --profile:all=sen_gcc -o "sen/*:with_examples=True"
```

`sen_gcc` targets Linux and detects the architecture, so the same pair of commands works on
