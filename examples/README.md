...
conan build . --profile:all=sen_gcc_x86
```
...
already open, so you can inspect the ticking `myCounter` object directly. The other examples
run the same way from the YAML files under `config/`, as each walkthrough shows; the path
segments (`build/gcc/Release`) follow the profile you built with.
