...
TEST(RecordingMergerTest, MergeFailsWhenTheOutputIsAlsoAnInput)
{
  ...
  writeKeyframeRecording("first", tempDir, {makeTime(10)});
  writeKeyframeRecording("second", tempDir, {makeTime(20)});

  const auto firstArchive = makeArchivePath("first", tempDir);
  auto settings = makeMergeSettings({firstArchive, makeArchivePath("second", tempDir)}, firstArchive);
...
TEST(RecordingMergerTest, MergeFailsWhenTheOutputAlreadyHoldsAnArchive)
{
  ...
  const auto mergedArchive = makeArchivePath("merged_twice", tempDir);
  auto settings =
    makeMergeSettings({makeArchivePath("first", tempDir), makeArchivePath("second", tempDir)}, mergedArchive);
  mergeRecordings(settings);
...
TEST(RecordingMergerTest, MergeReplacesAnExistingArchiveWhenForced)
{
  ...
  const auto mergedArchive = makeArchivePath("merged_forced", tempDir);
  auto settings =
    makeMergeSettings({makeArchivePath("first", tempDir), makeArchivePath("second", tempDir)}, mergedArchive);
  mergeRecordings(settings);
...
TEST(RecordingMergerTest, MergeReportsProgressFromZeroToComplete)
{
  ...
  writeKeyframeRecording("first", tempDir, {makeTime(10), makeTime(20)});
  writeKeyframeRecording("second", tempDir, {makeTime(30)});

  auto settings = makeMergeSettings({makeArchivePath("first", tempDir), makeArchivePath("second", tempDir)},
                                    makeArchivePath("merged_with_progress", tempDir));

  std::vector<std::pair<std::size_t, std::size_t>> reports;
  settings.progressReporter = [&reports](std::size_t bytesDone, std::size_t bytesTotal)
  { reports.emplace_back(bytesDone, bytesTotal); };

  mergeRecordings(settings);

  ASSERT_FALSE(reports.empty());
  EXPECT_EQ(reports.front().first, 0U);
  EXPECT_GT(reports.front().second, 0U);
  EXPECT_EQ(reports.back().first, reports.back().second);

  for (std::size_t i = 1U; i < reports.size(); ++i)
  {
    EXPECT_EQ(reports[i].second, reports.front().second);
    EXPECT_GE(reports[i].first, reports[i - 1U].first);
    EXPECT_LE(reports[i].first, reports[i].second);
  }
}
...
TEST(RecordingMergerTest, MergeThrottlesProgressToWholePercents)
{
  ...
  writeKeyframeRecording("first", tempDir, {makeTime(10), makeTime(20), makeTime(30)});
  writeKeyframeRecording("second", tempDir, {makeTime(40), makeTime(50), makeTime(60)});

  auto settings = makeMergeSettings({makeArchivePath("first", tempDir), makeArchivePath("second", tempDir)},
                                    makeArchivePath("merged_throttled", tempDir));

  std::size_t reportCount = 0U;
  settings.progressReporter = [&reportCount](std::size_t, std::size_t) { ++reportCount; };

  mergeRecordings(settings);

  EXPECT_GT(reportCount, 0U);
  EXPECT_LE(reportCount, 101U);
}
...
TEST(RecordingMergerTest, MergeWithoutAProgressReporterProducesTheSameArchive)
{
  ...
  const auto reported = makeArchivePath("merged_reported", tempDir);
  auto withReporter =
    makeMergeSettings({makeArchivePath("first", tempDir), makeArchivePath("second", tempDir)}, reported);
  withReporter.progressReporter = [](std::size_t, std::size_t) {};
  mergeRecordings(withReporter);

  const auto silent = makeArchivePath("merged_silent", tempDir);
  auto withoutReporter =
    makeMergeSettings({makeArchivePath("first", tempDir), makeArchivePath("second", tempDir)}, silent);
  mergeRecordings(withoutReporter);

  Input reportedInput(reported, kernel.getTypes());
  Input silentInput(silent, kernel.getTypes());
  EXPECT_EQ(reportedInput.getSummary().keyframeCount, silentInput.getSummary().keyframeCount);
  EXPECT_EQ(reportedInput.getSummary().firstTime, silentInput.getSummary().firstTime);
