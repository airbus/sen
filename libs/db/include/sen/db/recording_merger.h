...
/// Reports merge progress. Both values are in bytes of input, and the reporter is called only when
/// the whole-percent figure changes, so it is safe to do visible work in it. \ingroup db
using RecordingMergeProgressReporter = std::function<void(std::size_t bytesDone, std::size_t bytesTotal)>;
...
struct RecordingMergeSettings
{
  ...
  /// Replace a recording archive already present at the output path.
  bool force = false;
  /// Optional; no progress is reported when it is empty.
  RecordingMergeProgressReporter progressReporter;
};
