...
{
public:
  explicit MergedRuntimeEntry(v1::RuntimeDataEntry data);
...
public:
  ...
  void readInto(MergedRecording& mergedRecording) const;

private:
  [[nodiscard]] std::vector<MergedRuntimeEntry> readRuntimeEntries(ObjectIdMapper& objectIdMapper,
                                                                   MergedObjectRegistry& objectRegistry) const;
  [[nodiscard]] std::vector<MergedAnnotationEntry> readAnnotationEntries() const;
...
[[nodiscard]] v1::Keyframe readKeyframe(const v1::CompressedKeyframe& entry)
{
  ...
  uncompressBuffer(entry.buffer, buffer, entry.decompressedSize);
...
MergedRuntimeEntry::MergedRuntimeEntry(v1::RuntimeDataEntry data)
  : time_(readEntryTime(data))
  , isKeyframe_(isKeyframeEntry(data))
  , objectId_(readObjectId(data))
  , data_(std::move(data))
{
}
                                                                           ...
                                                                           MergedObjectRegistry& objectRegistry) const
{
  ...
  while (getCurrentFileOffset(file.get(), runtimePath) < fileSize)
  {
    ...
    if (applyTimeShift)
    {
      shiftRuntimeData(data, inputPlan_.timeShift);
    }
...
std::vector<MergedAnnotationEntry> RecordingArchiveReader::readAnnotationEntries() const
{
  const auto& archivePath = inputPlan_.archivePath;
...
void RecordingArchiveReader::readInto(MergedRecording& mergedRecording) const
{
  ...
  auto entries = readRuntimeEntries(objectIdMapper, mergedRecording.objectRegistry_);
  mergedRecording.runtimeEntries_.insert(mergedRecording.runtimeEntries_.end(),
                                         std::make_move_iterator(entries.begin()),
                                         std::make_move_iterator(entries.end()));

  auto annotations = readAnnotationEntries();
...
void updateObjectSnapshotUseTimeRange(ObjectUseTimeRange& objectUseTimeRange,
                                      const v1::Keyframe& keyframe,
                                      const TimeStamp& time)
{
  for (const auto& snapshot: keyframe.objects)
  {
    objectUseTimeRange.update(snapshot.objectId, time);
  }
}
...
void updateObjectSnapshotUseTimeRange(ObjectUseTimeRange& objectUseTimeRange,
                                      ...
                                      const TimeStamp& time)
{
  updateObjectSnapshotUseTimeRange(objectUseTimeRange, readKeyframe(entry), time);
}

void MergedRuntimeEntry::updateObjectUseTimeRange(ObjectUseTimeRange& objectUseTimeRange) const
{
  std::visit(
    Overloaded {[&](const v1::PropertyChange& data) { objectUseTimeRange.update(data.objectId, time_); },
                ...
                [&](const v1::Keyframe& data) { updateObjectSnapshotUseTimeRange(objectUseTimeRange, data, time_); },
                [&](const v1::CompressedKeyframe& data)
                { updateObjectSnapshotUseTimeRange(objectUseTimeRange, data, time_); },
                [](const auto&) {}},
    data_);
}
...
void MergedRecording::sortRuntimeEntries()
{
  ...
  std::stable_sort(runtimeEntries_.begin(),
                   runtimeEntries_.end(),
                   [&objectUseTimeRange](const auto& lhs, const auto& rhs)
                   {
                     const auto lhsTime = lhs.getSortTime(objectUseTimeRange);
                     const auto rhsTime = rhs.getSortTime(objectUseTimeRange);
                     if (lhsTime != rhsTime)
                     {
...
void mergeRecordings(const RecordingMergeSettings& settings)
{
  ...
  {
    RecordingArchiveReader(inputPlan).readInto(mergedRecording);
  }
