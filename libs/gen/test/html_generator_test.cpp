...
{
  var plain    : u8;
  var settable : u8 [writable];
  var fixed    : u8 [static];
  var frozen   : u8 [static_no_config];
}
)");

  const auto members = model()["types"]["t.Flags"]["members"];

  const auto hasFlag = [&members](const std::string& name, const std::string& flag)
  {
    for (const auto& member: members)
    {
      if (member["name"] == name)
      {
        const auto& flags = member["flags"];
        return std::find(flags.begin(), flags.end(), flag) != flags.end();
      }
    }
    ADD_FAILURE() << "no member named " << name;
    return false;
  };

  EXPECT_TRUE(hasFlag("settable", "Writable"));
  EXPECT_TRUE(hasFlag("plain", "Read Only"));
  EXPECT_TRUE(hasFlag("fixed", "Read Only"));
  EXPECT_TRUE(hasFlag("frozen", "Read Only"));

  EXPECT_TRUE(hasFlag("settable", "Dynamic"));
...
variant Told
{
  ...
  const auto& bare = types["t.Bare"]["table"];
  EXPECT_EQ(bare["header"].size(), 1U) << "a variant with no described arm should have one column";
  EXPECT_EQ(bare["header"][0], "Type");
  for (const auto& row: bare["rows"])
  {
    EXPECT_EQ(row.size(), 1U) << "row width must follow the header";
  }

  const auto& told = types["t.Told"]["table"];
  EXPECT_EQ(told["header"].size(), 2U) << "one described arm should bring the column back";
  EXPECT_EQ(told["header"][1], "Description");
  for (const auto& row: told["rows"])
  {
    EXPECT_EQ(row.size(), 2U) << "row width must follow the header";
  }
}
