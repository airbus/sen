...
[[nodiscard]] inja::json flagsOf(const sen::Property& prop)
{
  ...
  // Only dynamicRW has a public setter. staticRW can be set from code and configuration, which is
  // not the same thing, and calling it writable told readers a property had a setter it lacks.
...
void addFacts(const sen::CustomType& type, inja::json& entry, PrimitiveSink& sink)
{
  ...
  {
    // STL can describe each alternative, but most models do not, and a column of blanks reads as
    ...
    header = described ? inja::json::array({"Type", "Description"}) : inja::json::array({"Type"});
  }
