...
function setKindMode(mode){
  if(kindMode===mode)return;
  kindMode=mode;
  var seg=document.getElementById("seg");
  if(seg)seg.querySelectorAll("button").forEach(function(x){x.classList.toggle("on",x.dataset.v===mode);});
  var chips=document.getElementById("kindchips");
  if(chips)chips.hidden=(mode!=="data");
  refreshPkgCounts();
  applyFilter();
}
