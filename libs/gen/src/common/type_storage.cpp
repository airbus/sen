...
class TsAnnotationName final: protected sen::TypeVisitor
{
  SEN_NOCOPY_NOMOVE(TsAnnotationName)

public:
  static std::string get(const sen::Type& type)
  {
    TsAnnotationName visitor;
    type.accept(visitor);
    return visitor.result_;
  }
...
protected:
  ...
  // TimeStamp / Duration cross the wire as strings.
  void apply(const sen::DurationType& type) final
  {
    std::ignore = type;
    result_ = "string";
...
private:
  ...
  ~TsAnnotationName() final = default;
...
protected:
  ...
  {
    std::ignore = type;
    // Wire encoding is RFC 3339 with nanosecond fractional digits.
    result_ = R"("type": "string", "format": "date-time")";
  }
...
TypeStorage::TypeStorage(const sen::lang::TypeSet& typeSet, bool publicSymbols) noexcept
  : typeSet_(typeSet), nameSpace_(computeCppNamespace(typeSet)), publicSymbols_(publicSymbols)
{
}

std::tuple<std::string, std::string, std::string> TypeStorage::cppTypeName(const sen::Type& type) const
{
  return CppTypeName::get(type, nameSpace_);
}

template <typename T>
inja::json TypeStorage::toJsonList(const sen::Span<T>& list) const
{
...
inja::json TypeStorage::toJson(const sen::StructField& field) const
{
  ...
  data["tsAnnotationType"] = TsAnnotationName::get(*field.type);
...
inja::json TypeStorage::toJson(const sen::Enumerator& enumerator) const
{
  inja::json data;
...
inja::json TypeStorage::toJson(const sen::VariantField& field) const
{
  auto [absolute, relative, senRel] = cppTypeName(*field.type);
...
inja::json TypeStorage::toJson(const sen::StructType& type) const
{
  ...
  data["tsAnnotationType"] = TsAnnotationName::get(type);
  ...
  if (type.getParent())
  {
    ...
    data["tsParentAnnotationType"] = TsAnnotationName::get(*type.getParent().value());
...
inja::json TypeStorage::toJson(const sen::EnumType& type) const
{
  auto [absolute, relative, senRel] = cppTypeName(type);
...
inja::json TypeStorage::toJson(const sen::VariantType& type) const
{
  auto [absolute, relative, senRel] = cppTypeName(type);
...
inja::json TypeStorage::toJson(const sen::SequenceType& type) const
{
  auto [absolute, relative, senRel] = cppTypeName(type);
  ...
  data["pyElementAnnotationType"] = PythonAnnotationName::get(*type.getElementType());
  data["tsElementAnnotationType"] = TsAnnotationName::get(*type.getElementType());
  data["jsonElementSchemaType"] = JSONSchemaRef::get(*type.getElementType());
  data["namespace"] = computeCppNamespace(type);
...
inja::json TypeStorage::toJson(const sen::AliasType& type) const
{
  auto [absolute, relative, senRel] = cppTypeName(type);
  ...
  data["tsAliasedAnnotationType"] = TsAnnotationName::get(*type.getAliasedType());
  data["jsonSchemaType"] = JSONSchemaRef::get(type);
...
inja::json TypeStorage::toJson(const sen::OptionalType& type) const
{
  auto [absolute, relative, senRel] = cppTypeName(type);
  ...
  data["tsElementAnnotationType"] = TsAnnotationName::get(*type.getType());
  data["jsonSchemaType"] = JSONSchemaRef::get(type);
...
inja::json TypeStorage::toJson(const sen::QuantityType& type) const
{
  auto [absolute, relative, senRel] = cppTypeName(type);
  auto [absElem, relElem, senRelElem] = cppTypeName(*type.getElementType());
  ...
  data["hasUnit"] = type.getUnit().has_value();
  data["elementType"] = relElem;
  data["qualElementType"] = absElem;
  data["jsonElementSchemaType"] = JSONSchemaRef::get(*type.getElementType());
  data["namespace"] = computeCppNamespace(type);
...
inja::json TypeStorage::toJson(const sen::ClassType& type) const
{
  const auto& parents = type.getParents();
  ...
  if (!parents.empty())
  {
    for (const auto& parent: parents)
    {
      ...
      parentData["tsAnnotationType"] = TsAnnotationName::get(*parent);
...
inja::json TypeStorage::interfaceData(const sen::ClassType& type) const
{
  auto [absolute, relative, senRel] = cppTypeName(type);
...
inja::json TypeStorage::toJson(std::shared_ptr<const sen::Property> prop) const
{
  ...
  data["tsAnnotationType"] = TsAnnotationName::get(*prop->getType());
...
inja::json TypeStorage::toJson(const sen::Method& method) const
{
  inja::json data;
  ...
  const auto& relation = method.getPropertyRelation();
  inja::json relationData;
  if (std::holds_alternative<sen::PropertyGetter>(relation))
  {
    relationData["kind"] = "getter";
    relationData["propertyName"] = std::get<sen::PropertyGetter>(relation).property->getName();
  }
  else if (std::holds_alternative<sen::PropertySetter>(relation))
  {
    relationData["kind"] = "setter";
    relationData["propertyName"] = std::get<sen::PropertySetter>(relation).property->getName();
  }
  else
  {
    relationData["kind"] = "none";
  }
  data["propertyRelation"] = relationData;
  ...
  else
  {
    ...
    data["tsAnnotationType"] = TsAnnotationName::get(*retType);
...
inja::json TypeStorage::toJson(std::shared_ptr<const sen::Method> method) const { return toJson(*method); }

inja::json TypeStorage::toJson(std::shared_ptr<const sen::Event> ev) const
{
  inja::json data;
...
inja::json TypeStorage::toJson(const sen::Arg& arg) const
{
  ...
  data["tsAnnotationType"] = TsAnnotationName::get(*arg.type);
  data["jsonSchemaType"] = JSONSchemaRef::get(*arg.type);
  data["isOptional"] = arg.type->isOptionalType();
