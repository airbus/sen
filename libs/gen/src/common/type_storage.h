...
#ifndef SEN_LIBS_GEN_SRC_COMMON_TYPE_STORAGE_H
#define SEN_LIBS_GEN_SRC_COMMON_TYPE_STORAGE_H
...
// Per-TypeSet inja-data cache shared across every libs/gen emitter. Emits per-language
// type names (C++, Python, TypeScript) alongside JSON-Schema refs in a single pass.
class TypeStorage final
{
  SEN_NOCOPY_NOMOVE(TypeStorage)

public:
  // Takes the type set that we will
  // use to populate all the Json containers.
  explicit TypeStorage(const sen::lang::TypeSet& typeSet, bool publicSymbols = false) noexcept;
...
template <typename T>
const inja::json& TypeStorage::getOrCreate(const T& type)
{
...
const inja::json& TypeStorage::get(const sen::CustomType& type) const
{
  const std::string qualName(type.getQualifiedName());
  auto itr = map_.find(qualName);
  if (itr == map_.end())
...
#endif  // SEN_LIBS_GEN_SRC_COMMON_TYPE_STORAGE_H
