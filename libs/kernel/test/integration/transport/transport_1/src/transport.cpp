...
void TestClassImpl::testMethod4Impl(i16 arg1, std::promise<u16>&& promise)
{
  std::ignore = arg1;
  promise.set_value(0U);
}
...
bool TestClassImpl::prop5AcceptsSet(sen::Duration val) const
{
  std::ignore = val;
  return true;
}
...
TesterImpl::TesterImpl(std::string name, const sen::VarMap& args)
  : TesterBase(std::move(name), args), logger_(spdlog::stdout_color_mt("transport_1"))
{
}
...
void TesterImpl::registered(sen::kernel::RegistrationApi& api)
{
  ...
  localObj_ = std::make_shared<TestClassImpl>("obj1", prop6());
  ...
  remoteObj_->testMethod3(2.18f,
                          ...
                          {this,
                           [this](const auto& response)
                           {
                             if (response.isOk())
                             {
                               testMethod3Passed_ = response.getValue() == 2U;
                             }
                             checkRemote(6);
                           }});
  remoteObj_->testMethod4(-16,
                          {this,
                           [this](const auto& response)
                           {
                             if (response.isOk())
...
void TesterImpl::checkLocalStateImpl(std::promise<TestResult>&& promise)
{
  ...
  if (localFlags_.to_string() == "0000011")
  {
    ...
    if (checkProp<u8>(promise, "prop1", localObj_->getProp1(), prop1) &&
        checkProp<i64>(promise, "prop2", localObj_->getProp2(), prop2) && !eq(localObj_->getProp3(), prop3) &&
        !eq(localObj_->getProp4(), prop4) && !eq(localObj_->getProp5(), prop5) &&
        checkProp<std::string>(promise, "prop6", localObj_->getProp6(), prop6()) && !eq(localObj_->getProp7(), prop7))
    {
...
void TesterImpl::checkRemote(u32 flagToSet)
{
  ...
  // all bits except those from static properties should be set
  if (remoteFlags_.to_string() == "11111100011")
  {
    ...
    if (checkProp<u8>(promise_, "prop1", remoteObj_->getProp1(), prop1) &&
        checkProp<i64>(promise_, "prop2", remoteObj_->getProp2(), prop2) &&
        checkProp<f32>(promise_, "prop3", remoteObj_->getProp3(), prop3) && !eq(remoteObj_->getProp4(), prop4) &&
        !eq(remoteObj_->getProp5(), prop5) && !eq(remoteObj_->getProp6(), prop6()) &&
