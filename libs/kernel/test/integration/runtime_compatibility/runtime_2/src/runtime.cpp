...
constexpr i32 prop1 = 4;
constexpr f32 prop2 = -4.0;
constexpr bool prop3 = true;
const std::string& prop4()
{
  static const std::string prop4 = "-67.3";
  return prop4;
}

const TestSequence prop5 {3, 5, 6};
const TestArray& prop6()
{
  static const TestArray prop6 {6, 7, 8, 5, 4, 0};
  return prop6;
}

constexpr auto prop7 = TestEnum::elem4;
constexpr i32 prop8 = 1;
const std::string& prop9()
{
  static const std::string prop9 = "elem2";
  return prop9;
}

const TestStruct& prop10()
{
  static const TestStruct prop10 {3, "", -5.0};
  return prop10;
}

const TestVariant prop11 {13U};
const TestQuantity& prop12()
{
  static const TestQuantity prop12 = 12.0;
  return prop12;
}

constexpr f32 prop13 = 4.0;
const TestOptional& prop14()
{
  static const TestOptional prop14 = -56;
  return prop14;
}

constexpr u32 prop15 = 77U;
...
std::string TestClassImpl::testMethodImpl(const std::string& arg2, f64 arg1)
{
  std::ignore = arg1;
  std::string result = arg2;
  return result;
}

void TestClassImpl::emitEventsImpl() { testEvent(1, 4.6, sen::Emit::onCommit); }
    ...
    [this](const auto& iterators)
    {
      for (auto it = iterators.typedBegin; it != iterators.typedEnd; ++it)
      {
        ...
        if ((*it)->asObject().getId() != localObj_->getId())
        {
          ...
          remoteObj_
            ->onTestEvent({this,
                           [this](i32 arg2, TestOptional arg1)
                           {
                             testEventPassed_ = arg2 == 2 && arg1 == TestOptional(3);
                             checkRemote(16);
                           }})
            .keep();
          break;
...
void TesterImpl::doTestsImpl(std::promise<TestResult>&& promise)
{
  ...
  remoteObj_->testMethod("hello",
                         -5.8,
                         {this,
                          [this](const auto& res)
                          {
                            // method should return error because there are more args in the writer schema
                            testMethodPassed_ = res.isError();
                            checkRemote(15);
                          }});
...
void TesterImpl::checkLocalStateImpl(std::promise<TestResult>&& promise)
{
  if (localFlags_.all())
  {
    ...
    if (checkProp<i32>(promise, "prop1", localObj_->getProp1(), prop1) &&
        checkProp<f32>(promise, "prop2", localObj_->getProp2(), prop2) &&
        checkProp<bool>(promise, "prop3", localObj_->getProp3(), prop3) &&
        checkProp<f64>(promise, "prop4", stod(localObj_->getProp4()), stod(prop4())) &&
        ...
        checkProp<i32>(promise, "prop8", localObj_->getProp8(), prop8) &&
        checkProp<std::string>(promise, "prop9", localObj_->getProp9(), prop9()) &&
        ...
        checkProp<f32>(promise, "prop13", localObj_->getProp13(), prop13) &&
        checkProp<TestOptional>(promise, "prop14", localObj_->getProp14(), prop14()) &&
        checkProp<u32>(promise, "prop15", localObj_->getProp15(), prop15))
    {
...
void TesterImpl::checkRemote(u32 flagToSet)
{
  ...
  if (remoteFlags_.all())
  {
    if (checkProp<i32>(promise_, "prop1", remoteObj_->getProp1(), prop1) &&
        checkProp<f32>(promise_, "prop2", remoteObj_->getProp2(), prop2) &&
        checkProp<bool>(promise_, "prop3", remoteObj_->getProp3(), prop3) &&
        checkProp<f64>(promise_, "prop4", stod(remoteObj_->getProp4()), stod(prop4())) &&
        ...
        checkProp<i32>(promise_, "prop8", remoteObj_->getProp8(), prop8) &&
        checkProp<std::string>(promise_, "prop9", remoteObj_->getProp9(), prop9()) &&
        ...
        checkProp<f32>(promise_, "prop13", remoteObj_->getProp13(), prop13) &&
        checkProp<TestOptional>(promise_, "prop14", remoteObj_->getProp14(), prop14()) &&
        checkProp<u32>(promise_, "prop15", remoteObj_->getProp15(), prop15) &&
