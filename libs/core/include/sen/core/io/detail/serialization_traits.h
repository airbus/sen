...
template <typename T>
[[nodiscard]] constexpr bool isPureIntegral() noexcept
{
    return std::is_same_v<int8_t, T>  || std::is_same_v<uint8_t, T>  ||
...
template <typename T>
[[nodiscard]] constexpr bool isNumeric() noexcept
{
  return isPureIntegral<T>() || (std::is_floating_point_v<T> && std::numeric_limits<T>::is_iec559);
}

/// True if T can be directly written to a buffer (without transformation) if endianness matches.
template <typename T>
[[nodiscard]] constexpr bool isBasic() noexcept
{
  return isNumeric<T>() || std::is_same_v<char, T> || std::is_same_v<unsigned char, T>;
}

/// True if T can be memcopied without gaps in a sequence
/// of contiguous memory (like std::vector or sen::StaticVector)
template <typename T>
[[nodiscard]] constexpr bool allowsContiguousIO() noexcept
{
  constexpr bool isBasicBuiltinType = isBasic<T>() || std::is_enum_v<T>;
  constexpr bool isTrivialType = std::is_trivially_copyable_v<T> && !std::is_class_v<T>;
  constexpr bool wouldNotRequireSwap = hostIsLittleEndian() || sizeof(T) == 1U;

  return !std::is_same_v<T, bool> && (isBasicBuiltinType || isTrivialType) && wouldNotRequireSwap;
}
