...
| `static_file_server.{h,cpp}` | `StaticFileServer`, `BundleRegistry` | Optional. Serves static assets from URL prefixes registered at runtime via the control bus. Used by `web_explorer` to ship its frontend bundle on the same origin as the WebSocket. |
...
- **Static-file consumers**: `web_explorer` is the canonical caller -- subscribe to the
