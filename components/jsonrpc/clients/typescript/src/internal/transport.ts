...
const defaultOpenAttemptTimeoutMs = 10_000;

interface TransportOptions {
  url: string;
  defaultTimeoutMs?: number;
  reconnect?: ReconnectOptions;
  /** Deadline for a single WebSocket handshake attempt. An attempt that neither opens
   *  nor closes within this time is abandoned and the reconnect schedule moves on.
   *  Without it, a server that accepts TCP but never answers the handshake would wedge
   *  the client in "reconnecting" forever. Default 10 seconds. */
...
export class Transport {
  ...
  private openAttemptTimer: ReturnType<typeof setTimeout> | null = null;
  private readonly openAttemptTimeoutMs: number;
  ...
  constructor(options: TransportOptions) {
    ...
    this.openAttemptTimeoutMs = options.openTimeoutMs ?? defaultOpenAttemptTimeoutMs;
    this.reconnect = {
  ...
  private openSocket(): void {
    ...
    // Guard every event by socket identity: an abandoned attempt (see the handshake
    // deadline below) must not be able to flip the state of its successor.
    ...
    // A handshake that neither opens nor closes within the deadline is abandoned and
    // counted as a failed attempt, so the reconnect schedule keeps going.
    this.openAttemptTimer = setTimeout(() => {
      this.openAttemptTimer = null;
      if (this.socket !== socket || this.state === "open" || this.state === "closed") return;
      try {
        socket.close();
      } catch {
        // the socket may reject close() while still connecting; abandoning it is enough
      }
      this.handleClose({ code: 4000, reason: "handshake timed out" });
    }, this.openAttemptTimeoutMs);
  }

  private clearOpenAttemptTimer(): void {
    if (this.openAttemptTimer !== null) {
      clearTimeout(this.openAttemptTimer);
      this.openAttemptTimer = null;
    }
  }
  ...
  private readonly handleClose = (event: { code: number; reason: string }): void => {
    this.clearOpenAttemptTimer();
    if (this.state === "closed") return;
    this.socket = null;
    ...
    if (shouldRetry) {
      ...
      if (this.hasEverConnected) {
        ...
        this.rejectOpen(new TransportError(`initial connect failed (${this.url})${detail}`, initOpts));
      }
    }
  };
