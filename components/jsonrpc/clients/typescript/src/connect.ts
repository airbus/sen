...
export interface ClientOptions {
   ...
   *  Also used as the deadline for each reconnect handshake attempt: an attempt that
   *  neither opens nor closes within this time is abandoned and the next one is tried. */
...
export async function connect(options: ClientOptions): Promise<Client> {
  ...
  if (options.openTimeoutMs !== undefined) transportOpts.openTimeoutMs = options.openTimeoutMs;
