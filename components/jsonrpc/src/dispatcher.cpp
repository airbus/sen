...
void Dispatcher::processBatch()
{
  ...
  {
    handleInbound(std::move(batch.at(i)));
  }
                                   ...
                                   std::chrono::steady_clock::time_point now)
{
  ...
  // The dirty values are read here, at flush time, so no callback lock is held while
  // the object's reader lock is taken (see ObjectSubs::dirtyProperties). A vanished
  // object simply drops its dirty set: its removal is reported separately.
  auto object = subs.object.lock();
  if (!object)
  {
    subs.dirtyProperties.clear();
    return;
  }
  ...
  std::string envelope;
  envelope.reserve(128U + subs.dirtyProperties.size() * 64U);
  ...
  for (const auto& [propertyName, property]: subs.dirtyProperties)
  {
    if (!firstValue)
    {
      envelope.push_back(',');
    ...
    const auto value = varToWireJson(object->getPropertyUntyped(property), *property->getType());
...
void Dispatcher::handleInbound(InboundMessage&& msg)
{
  ...
  std::visit(
    [this, connId](auto&& payload)
    {
      ...
      {
        handleClientConnected(connId, std::forward<decltype(payload)>(payload));
      }
