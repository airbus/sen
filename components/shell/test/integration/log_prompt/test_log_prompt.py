...
class TestLogPrompt(unittest.TestCase):
    ...
    def test_prompt_after_async_log(self) -> None:
        ...
        try:
            # Waits for the log rather than for a window: the second process has to start, reach
            # the discovery hub and connect, which takes longer than one read on a loaded machine.
            # The deadline only bounds the failure; a healthy run leaves the loop on the first pass.
            target_log: str = "discovery hub accepted client"
            deadline: float = time.monotonic() + 30.0
            out2: str = ""
            while target_log not in out2 and time.monotonic() < deadline:
                out2 += self.shell.read_output()

            self.assertIn(target_log, out2, f"Connection log not found in shell. Output:\n{out2}")

            # The prompt is redrawn after the log, so read on until the output settles.
            out2 += self.shell.read_output()
